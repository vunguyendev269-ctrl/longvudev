--[[
    KATAKURI COORDINATOR CLIENT V5.6 - BATCH100 HOP + CAKE STAGE/MIRROR FIX
    ...
    Design goals:
      ...
      - Server Browser max 200 pages; ONLY 4/5/6/7-player servers, priority 4 -> 5 -> 6 -> 7.
...
-- ============================================================================
-- SERVER BROWSER V5.6 — BATCH100 / 50 CONCURRENT / 200 MAX
-- ============================================================================
-- Max page          = 200
-- Page mỗi batch    = 100
-- Request song song = 50
-- Timeout batch     = 5 giây
-- Filter            = chỉ 4 / 5 / 6 / 7 players
-- Chỉ thử hop SAU KHI batch 100 page hoàn tất hoặc timeout.
-- Priority tuyệt đối: 4p -> 5p -> 6p -> 7p.
--
-- Worker pool kiểu Cyborg:
--   + 50 worker dùng chung một cursor.
--   + Worker nào xong request sẽ lấy page kế tiếp ngay.
--   + Tối đa 50 InvokeServer(page) đồng thời.
--
-- ROOM FIRST / WebSocket / candidate claim vẫn giữ nguyên.

local BROWSER_BATCH_PAGES = 100
local BROWSER_WORKERS = 50
local BROWSER_BATCH_MAX_WAIT = 5.0
...
local function makeCandidate(jobId, info)
    ...
    if not ALLOWED_BROWSER_PLAYERS[count] then
        return nil
    end
...
local function shuffleForAccount(list)
    -- Chỉ đảo JobId TRONG CÙNG player-count.
    -- Priority 4 -> 5 -> 6 -> 7 không thay đổi.
...
local function fetch100PageBatch(batchStart, rawTotals, usableTotals)
    ...
    local workerCount = math.min(BROWSER_WORKERS, #pages)
    local scanStartedAt = os.clock()

    local function processPageData(data)
        if not batchOpen or type(data) ~= "table" then
            return
        end
        ...
        for jobId, info in pairs(data) do
            ...
            if ALLOWED_BROWSER_PLAYERS[count] then
                rawTotals[count] += 1

                local candidate = makeCandidate(jobId, info)
                if candidate and not seenInBatch[candidate.jobId] then
                    seenInBatch[candidate.jobId] = true
    ...
    local function worker()
        while batchOpen and State.Running do
            if State.PendingAssignment or State.RoomHint then
                break
            end

            cursor += 1
            local page = pages[cursor]
            if not page then
                break
            end
    ...
    -- Không thử hop giữa lúc scan.
    -- Chỉ kết thúc batch khi đủ 100 page, timeout 5s, hoặc ROOM FIRST xuất hiện.
    while State.Running
        and workersDone < workerCount
        and os.clock() - scanStartedAt < BROWSER_BATCH_MAX_WAIT
    ...
    local timedOut = workersDone < workerCount
        and not State.PendingAssignment
        and not State.RoomHint

    -- Request trả sau deadline không được ghi thêm vào batch đã đóng.
    batchOpen = false
...
function Browser.scanAndHop()
    ...
    for batchStart = 1, Config.BrowserMaxPages, BROWSER_BATCH_PAGES do
        ...
        setStatus(
            ...
            "SCANNING",
            string.format(
                "batch=%d-%d/%d | 4p=%d | 5p=%d | 6p=%d | 7p=%d",
                batchStart,
                batchEnd,
                Config.BrowserMaxPages,
                usable[4],
        ...
        local result = fetch100PageBatch(batchStart, raw, usable)
        ...
        setStatus(
            "Server Browser batch ready",
            "SCANNING",
            string.format(
                "pages=%d-%d | done=%d/%d | failed=%d | workers=%d%s | 4p=%d 5p=%d 6p=%d 7p=%d",
                result.BatchStart,
                result.BatchEnd,
                ...
                result.FailedPages,
                result.WorkerCount,
                result.TimedOut and " | TIMEOUT" or "",
                usable[4],
                usable[5],
                usable[6],
                usable[7]
        ...
        local batch = result.Candidates

        -- Chỉ sau khi batch kết thúc/timeout mới bắt đầu thử hop.
        -- Priority tuyệt đối: 4p -> 5p -> 6p -> 7p.
        for _, playersCount in ipairs({4, 5, 6, 7}) do
    ...
    setStatus(
        "No usable 4/5/6/7 server - rescanning",
        "SCOUT",
        string.format(
            "RAW 4p=%d | 5p=%d | 6p=%d | 7p=%d",
